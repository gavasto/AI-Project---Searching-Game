  ...
  ;; setup STARTING POINT
   ask patches [
    if pxcor = 18 and pycor = 0
      [ set pcolor orange ]
    if pxcor = 0 and pycor = 18
      [ set pcolor orange ]
  ]
  ;; setup agent in starting point
  ask n-of 2 patches with [pcolor = orange and not any? other turtles-here][sprout-agents 1 [
    set shape "plant"
    set color green
    set size 2
    ]
...
to setup-random
  ask n-of 100 patches [
    if pxcor >= (- halfedge) and pxcor <=  halfedge and pycor >= (- halfedge) and pycor <= halfedge
     [set pcolor brown]
  ]
end
@#$#@#$#@
